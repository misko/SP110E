...
class Driver:
    ...
    async def connect(
        ...
        self._client = BleakClientBlueZDBus(
            mac_address, timeout=timeout, adapter=adapter
        )
        await self._client.connect()
        self.c = self._client.services.get_characteristic(self.CHARACTERISTIC)
        await self._client.start_notify(self.c, self._callback_handler)
        if auto_read:
    ...
    async def send_command(
        self, command_byte: hex, data_bytes: [hex, hex, hex] = (0x00, 0x00, 0x00)
    ):
        """Send command with data."""
        if type(data_bytes) not in [list, tuple]:
            data_bytes = (data_bytes, 0, 0)
        data_to_write = bytearray(tuple(data_bytes) + tuple([command_byte]))
        # self.c = self._client.services.get_characteristic(self.CHARACTERISTIC)
        await self._client.write_gatt_char(self.c, data_to_write, response=True)
    ...
    async def write_parameters(
        self, parameters: dict, auto_read: bool = True
    ) -> Union[dict, None]:
    ...
    async def write_parameter(
        self, parameter: str, value: Any, auto_read: bool = True
    ) -> Union[dict, None]:
    ...
    def _callback_handler(self, data: bytearray):
